...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  "Plus 3 numbers" should "add three number values and return a number" in {
    val e1 = N(1)
    val e2 = N(2)
    val e3 = N(3)
    val e4 = eval(Binary(Plus, Binary(Plus, e1, e2),e3))
    assert(e4 === N(6))
  ...
  "Boolean_Eq" should "return false if two boolean values are not the same" in {
    val e1 = B(false)
    val e2 = B(true)
    val e3 = eval(Binary(Eq, e1, e2))
    assert(e3 === B(false))
  }
  "Strings_Eq" should "return true if two strings are the same" in {
    val e1 = S("abc")
    val e2 = S("abc")
    val e3 = eval(Binary(Eq, e1, e2))
    assert(e3 === B(true))
  }
  "int&boolean_Eq" should "return true if true == 1 or false == 0" in {
    val e1 = N(0)
    val e2 = N(1)
    val e3 = eval(Binary(Eq, e1, B(false)))
    val e4 = eval(Binary(Eq, B(true), e2))
    assert(e3 === B(true))
    assert(e4 === B(true))
  }
